...
protocol PTManagerDelegate {
    ...
    class PTManager: NSObject {
        ...
        let name: String?
        fileprivate var notConnectedQueue: DispatchQueue
        let deviceSerialNumber: String
        
        init(name: String, deviceSerialNumber: String) {
            print("NEW THING")
            
            self.name = name
            self.notConnectedQueue = DispatchQueue(label: name)
    ...
    extension PTManager {
        ...
        fileprivate func startListeningForDevices() {
            ...
            nc.addObserver(forName: NSNotification.Name.PTUSBDeviceDidAttach, object: PTUSBHub.shared(), queue: nil) { (note) in
                ...
                let properties = (note.userInfo?["Properties"] as? NSDictionary)
                let serialNumber = properties?["SerialNumber"] as? String
                
                print(serialNumber)
                
                if serialNumber == self.deviceSerialNumber {
